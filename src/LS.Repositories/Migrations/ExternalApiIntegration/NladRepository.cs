            ...
            foreach (var rejectionObject in nladResponse.Body)
                ...
                foreach (var rejectionDetail in rejectionObject)
                    ...
                    if (rejectionDetailLine == 3)
                        {
                        if (rejectionDetail.Trim() == "AMS_FAILURE_ANALYSIS")
                            { NLADRejections.InvalidAddress = true; }
                        else if (rejectionDetail.Trim().Contains("TPIV_FAIL"))
                            {
                            NLADRejections.FailedIdentity = true;
                            if (rejectionDetail.Trim() == "TPIV_FAIL_NAME_SSN4" || rejectionDetail.Trim() == "TPIV_FAIL_IDENTITY_NOT_FOUND")
                                { NLADRejections.FailedIdentityNameSSN = true; }
                            if (rejectionDetail.Trim() == "TPIV_FAIL_DOB")
                                { NLADRejections.FailedIdentityDOB = true; }
                            }
                        else if (rejectionDetail.Trim() == "DUPLICATE_PRIMARY_ADDRESS")
                            { NLADRejections.InvalidAddress = true; NLADRejections.DuplicateAddress = true; }
                        else if (rejectionDetail.Trim() == "DUPLICATE_SUBSCRIBER")
                            { NLADRejections.DuplicateSubscriber = true; }
                        else if (rejectionDetail.Trim() == "CANNOT_TRANSFER_WITHIN_60_DAYS")
                            { NLADRejections.CantTransfer = true; }
                        else if (rejectionDetail.Trim() == "TPIV Webservice")
                            { } //Do Nothing - just catching these so doesn't get labelled as Unknown
                        else if (rejectionDetail.Trim() == "Subscriber")
                            { } //Do Nothing - just catching these so doesn't get labelled as Unknown
                        else if (rejectionDetail.Trim() == "Address")
                            { } //Do Nothing - just catching these so doesn't get labelled as Unknown
                        else
                            { NLADRejections.UnknownRejection = true; }
                        }
                    }
                }
