...
namespace LS.Repositories.ExternalApiIntegration
{
    public class BudgetRESTRepository
    {
        ...
        public async Task<ServiceProcessingResult<GetOrderResponse>> GetOrder(string orderid)
        {
            var result = new ServiceProcessingResult<GetOrderResponse> { IsSuccessful = true };
            try
            {
                var cookieContainer = new CookieContainer();
                using (var handler = new HttpClientHandler { CookieContainer = cookieContainer })
                {
                        ...
                        })
                    {
                        ...
                        var getOrderRequestData = new IGetOrderRequest
                        {
                            OrderID = orderid
                        };
                        ...
                        var logEntry = CreateInitialGetOrderApiLogEntry(jsonRequestData);

                        var response = await httpClient.GetAsync(OrderEndpoint + orderid);


                        if (response.Content == null)
                        {
                            FinishLogEntry(logEntry, "No response content");
                            result.IsSuccessful = false;
                            result.Error = new ProcessingError("An error occurred calling Budget REST API", "An error occurred calling Budget REST API. Please try again, if error persists contact support", true);
                            return result;
                        }
                        ...
                        try
                        {
                            ...
                            var jsonContent = JsonConvert.DeserializeObject<GetOrderResponse>(apiResponse);

                            result.Data = new GetOrderResponse
                            {
                                IsError = jsonContent.IsError,
                                ErrorMessage = jsonContent.ErrorMessage,
                                ErrorCode = jsonContent.ErrorCode,
                                BudgetMobileID = jsonContent.BudgetMobileID,
                                OrderID = jsonContent.OrderID,
                                LSOrderID = jsonContent.LSOrderID,
                                HEX = jsonContent.HEX,
                                DEC = jsonContent.DEC,
                                IMSI = jsonContent.IMSI,
                                AccountStatus = jsonContent.AccountStatus,
                                OrderStatusCode = jsonContent.OrderStatusCode,
                                CA_PDF = jsonContent.CA_PDF,
                                CA_Reserved = jsonContent.CA_Reserved,
                                CA_TransID = jsonContent.CA_TransID,
                                CA_Status = jsonContent.CA_Status,
                                CA_AccountType = jsonContent.CA_AccountType,
                                CA_DatePDFUpload = jsonContent.CA_DatePDFUpload,
                                DateFulfilled = jsonContent.DateFulfilled,
                                IsDeletedRejected=jsonContent.IsDeletedRejected,
                                Notes=jsonContent.Notes,
                                Deleted=jsonContent.Deleted
                        ...
                        catch (Exception exception)
                        {
                            ...
                            return new ServiceProcessingResult<GetOrderResponse>
                            {
                                Error = new ProcessingError("An error occurred processing BudgetREST API response", "An error occurred processing BudgetREST API response. Please try again, if error persists contact support", true),
                                IsSuccessful = false
                            };
