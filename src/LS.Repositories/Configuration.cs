...
namespace LS.Repositories.Migrations
{
    ...
    {
        internal static readonly string BudgetMobileName = "Budget Mobile";
        ...
        protected override void Seed(ApplicationDbContext context)
        {
            ...
            CreateNladPhoneNumbersIfNoneExist(context, company);
            CreateSalesTeamAndGroupsIfDoesNotExist(context, company, user);
            //CreateSystemStatusIfNoneExist(context);
            context.SaveChanges();
        }
        ...
        private static void LoadEntitesToBeUpdated(ApplicationDbContext context)
        {
            ...
            context.SystemStatus.Load();
            context.Users.Load();
            context.CaliPhoneNumbers.Load();
            context.NladPhoneNumbers.Load();
        ...
        //private static void CreateSystemStatusIfNoneExist(ApplicationDbContext context)
        //{
        //    if (context.SystemStatus.Any())
        //    {
        //        return;
        //    }
        //    context.SystemStatus.AddOrUpdate(x => x.Id,
        //    new SystemStatus() {Id= Guid.NewGuid().ToString(), System = "", Status = "UP",Details = "",Eta="",ExternalSystem = true,
        //        SortOrder =10,Display=true,DateCreated=DateTime.UtcNow,DateModified=DateTime.UtcNow, IsDeleted=false },
        //   new SystemStatus()
        ...
        //   {
        //       Id = Guid.NewGuid().ToString(),
        //       System = "Street Fleet Portal",
        //       Status = "UP",
        //       Details = "",
        //       Eta = "",
        //       SiteStatusColName = "StreetFleet",
        //       ExternalSystem = false,
        //       SortOrder = 30,
        //       Display = true,
        //       DateCreated = DateTime.UtcNow,
        ...
        //       Details = "",
        //       Eta = "",
        //       SiteStatusColName ="Fullfillment",
        //       ExternalSystem = false,
        //       SortOrder = 60,
        //       Display = true,
        //       DateCreated = DateTime.UtcNow,
        ...
        //                 {
        //                     Id = Guid.NewGuid().ToString(),
        //                     System = "Public Website (CA)",
        //                     Status = "UP",
        //                     Details = "",
        //                     Eta = "",
        //                     SiteStatusColName = "California",
        //                     ExternalSystem = false,
        //                     SortOrder = 80,
        //                     Display = true,
        //                     DateCreated = DateTime.UtcNow,
        ...
        private static void CreateNladPhoneNumbersIfNoneExist(ApplicationDbContext context, Company company)
        {
            if (context.NladPhoneNumbers.Any())
            {
                return;
            }

            //Todo: For production change 100 to 9999. Long operation
            for(var i = 0; i < 100; i++)
            {
                var startingNumber = 3189080000;
                var phoneNumber = startingNumber + i;
                var nladPhoneNumberEntry = new NladPhoneNumber
                {
                    CompanyId = company.Id,
                    DateCreated = DateTime.UtcNow,
                    DateModified = DateTime.UtcNow,
                ...
                context.NladPhoneNumbers.Add(nladPhoneNumberEntry);
                context.SaveChanges();
            }
        ...
        private static void CreateExternaStorageCredentialsIfDoesntExist(ApplicationDbContext context, Company company)
        {
            if (!context.ExternalStorageCredentials.Any(sc => sc.Type == "Proof"))
            {
                ...
                {
                    CompanyId = companyId,
                    AccessKey = "AKIAIEJB6KJN6HBTXM3A",
                    SecretKey = "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",
                    Type = "Proof",
                    System = "AWS",
                    Path = "lifeline.proofs",
                    MaxImageSize = 640,
                    Id = Guid.NewGuid().ToString(),
                    DateCreated = DateTime.UtcNow,
                    DateModified = DateTime.UtcNow,
        ...
        private static void CreateSalesTeamAndGroupsIfDoesNotExist(ApplicationDbContext context, Company company, ApplicationUser user)
        {
            if (!context.Level1SalesGroups.Any())
            {
                ...
                salesTeam.CommissionLevel1UserId = level1SalesManager.Id;
                salesTeam.CommissionLevel2UserId = level2SalesManager.Id;
                salesTeam.CommissionLevel3UserId = level3SalesManager.Id;
